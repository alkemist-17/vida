...
func castToString(ctx *Context, args ...Value) (Value, error) {
	if len(args) > 0 {
		return &String{Value: args[0].String(), VTable: ctx.initialVTables[stringVT]}, nil
	}
	return &String{Value: EmptyString, VTable: ctx.initialVTables[stringVT]}, nil
}
	...
	if len(args) > 0 {
		...
		case *Object:
			...
			for k, val := range v.Value {
				pairs[i] = &Array{Value: []Value{&String{Value: k, VTable: ctx.initialVTables[stringVT]}, val}}
				i++
			}
