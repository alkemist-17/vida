...
func ioFWrite(ctx *Context, args ...Value) (Value, error) {
	if len(args) > 1 {
		...
		case *Object:
			...
			return &VidaError{Message: &String{Value: noOrClosedFH, VTable: ctx.initialVTables[stringVT]}}, nil
		case *FileHandler:
			if handler.IsClosed {
	...
	if len(args) > 2 {
		...
		case *Object:
			if fileHandler, ok := handler.Value[fileHandlerName].(*FileHandler); ok && !fileHandler.IsClosed {
				...
				return &VidaError{Message: &String{Value: noStringFormat, VTable: ctx.initialVTables[stringVT]}}, nil
			}
			return &VidaError{Message: &String{Value: noOrClosedFH, VTable: ctx.initialVTables[stringVT]}}, nil
		case *FileHandler:
			if formatstr, ok := args[1].(*String); ok {
