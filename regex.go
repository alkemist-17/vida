...
func regexpReplaceAll(ctx *Context, args ...Value) (Value, error) {
	if len(args) > 2 {
		...
		if okPatt && okIn && okRepl {
			...
			return &String{Value: re.ReplaceAllString(source.Value, replacement.Value), VTable: ctx.initialVTables[stringVT]}, nil
		}
	}
	return Nil, nil
}
...
func regexpReplaceAllLit(ctx *Context, args ...Value) (Value, error) {
	if len(args) > 2 {
		...
		if okPatt && okIn && okRepl {
			...
			return &String{Value: re.ReplaceAllLiteralString(source.Value, replacement.Value), VTable: ctx.initialVTables[stringVT]}, nil
		}
	}
	return Nil, nil
}
	...
	if len(args) > 2 {
		...
		if okPatt && okIn && okn {
			...
			for i, v := range result {
				arr.Value[i] = &String{Value: v, VTable: ctx.initialVTables[stringVT]}
			}
			return arr, nil
		}
	}
...
func regexpEscape(ctx *Context, args ...Value) (Value, error) {
	if len(args) > 0 {
		if input, ok := args[0].(*String); ok {
			return &String{Value: regexp.QuoteMeta(input.Value), VTable: ctx.initialVTables[stringVT]}, nil
		}
	}
	return Nil, nil
}
...
func regexpFindString(ctx *Context, args ...Value) (Value, error) {
	if len(args) > 1 {
		...
		if okPatt && okIn {
			...
			return &String{Value: re.FindString(input.Value), VTable: ctx.initialVTables[stringVT]}, nil
		}
	}
	return Nil, nil
}
