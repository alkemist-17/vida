...
func timeNow(ctx *Context, args ...Value) (Value, error) {
	...
	case 1:
		if f, ok := args[0].(*String); ok && f.Value == time.Local.String() {
			...
			if len(r) > 0 {
				return &String{Value: r, VTable: ctx.vtables[stringVT]}, nil
			}
		}
	case 2:
		if f, ok := args[0].(*String); ok {
			if l, ok := args[1].(*String); ok {
				...
				case time.Local.String():
					return &String{Value: time.Now().Local().Format(f.Value), VTable: ctx.vtables[stringVT]}, nil
				case time.UTC.String():
					return &String{Value: time.Now().UTC().Format(f.Value), VTable: ctx.vtables[stringVT]}, nil
				}
			}
		}
	}
	...
	if len(args) > 1 {
		if t, ok := args[0].(Time); ok {
			if f, ok := args[1].(*String); ok {
				return &String{Value: time.Time(t).Format(f.Value), VTable: ctx.vtables[stringVT]}, nil
			}
		}
	}
	return Nil, nil
...
func timeGetLocation(ctx *Context, args ...Value) (Value, error) {
	if len(args) > 0 {
		if t, ok := args[0].(Time); ok {
			return &String{Value: time.Time(t).Location().String(), VTable: ctx.vtables[stringVT]}, nil
		}
	}
	return Nil, nil
}
...
func timeCreateDuration(ctx *Context, v time.Duration) *Object {
	...
	o.Value["description"] = &String{Value: v.String(), VTable: ctx.vtables[stringVT]}
	return o
}
