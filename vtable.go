...
func (ctx *Context) loadStyleVT() {
	vt := &Object{Value: map[string]Value{
		...
		"bold":      NativeFunction(styleFlag("style.bold", func(s *Style) *Style { return s.Bold() })),
		"dim":       NativeFunction(styleFlag("style.dim", func(s *Style) *Style { return s.Dim() })),
		"italic":    NativeFunction(styleFlag("style.italic", func(s *Style) *Style { return s.Italic() })),
		"underline": NativeFunction(styleFlag("style.underline", func(s *Style) *Style { return s.Underline() })),
		"blink":     NativeFunction(styleFlag("style.blink", func(s *Style) *Style { return s.Blink() })),
		"reverse":   NativeFunction(styleFlag("style.reverse", func(s *Style) *Style { return s.Reverse() })),
		"strike":    NativeFunction(styleFlag("style.strike", func(s *Style) *Style { return s.Strike() })),
		"noReset":   NativeFunction(styleFlag("style.noReset", func(s *Style) *Style { return s.NoReset() })),
